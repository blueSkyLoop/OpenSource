...
static void append_referrer(weak_entry_t *entry, objc_object **new_referrer)
{
    ...
    // entry(哈希)， referrers(弱引用对象的指针集合）
    // 哈希 -> 数组[index]
    weak_referrer_t &ref = entry->referrers[index];
    ref = new_referrer; // 弱引用对象
    entry->num_refs++;
}
...
weak_entry_for_referent(weak_table_t *weak_table, objc_object *referent)
{
    ...
    while (weak_table->weak_entries[index].referent != referent) {
        // 找到哈希表的下标 -- entry --> 哈希冲突，为了找到唯一的index 所以+ 1
        index = (index+1) & weak_table->mask;
                        ...
                        id *referrer_id)
{
    ...
    if (!referent) return;
    // entry ： 通过 oldObject 在 weak_table 找到
                      ...
                      id *referrer_id, bool crashIfDeallocating)
{
    ...
    // 注册之前
    // 弱引用对象， 一个referent对象 ----> 对应一个 entry（哈希表）
    // weakTable表里，含有各种类，如：LGPerson LGDog, 为了保持数据不混乱
    // entry 数组 （哈希表）--- 防止冲突 --- 找到下标
    //  refreces
    weak_entry_t *entry;
    //  通过 referent 去哈希表找实体 entry
    // 对 entry 加入 weak 引用对象
    // 存储完毕， 散列表.weak表.entry.数组
    // 什么时候会释放？  在 delloc 底层的 sidetable_clearDeallocating
    if ((entry = weak_entry_for_referent(weak_table, referent))) {
        // 对实体进行拼接
        ...
        // 若之前没有这个实体，就创建一个，并加入散列表，和进行扩容
        // 散列表 含有-> entry -> weak 引用对象
...
//        entry -> referrers 是一个弱引用对象的地址集合
        // 下面 用 “**” 就是去对象出来
        objc_object **referrer = referrers[i];
        if (referrer) {
            // 如果传入的对象 referent 与 在entry.referrers遍历后的 *referrer匹配上，就赋值为nil
