 ...
 public:
    ...
    void attachLists(List* const * addedLists, uint32_t addedCount) {
        ...
        if (hasArray()) {
            ...
            // 1.newCount = 获取旧数组的数量 并与 新增加的数量 进行合并
...
struct objc_class : objc_object {
    ...
    /*
     ...
     SEL、 IMP 最终会组成一张哈希表，通过哈希表查找会非常之快
     查找方法的时候，会首先找到cache , 找到就会直接返回；
     如果没有就会通过很漫长的过程去创建方法，并再存在cache里
     */
    cache_t cache;        // 16 bit        // formerly cache pointer and vtable
