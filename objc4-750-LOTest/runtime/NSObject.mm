...
storeWeak(id *location, objc_object *newObj)
{
 ...
 retry:
    ...
    // 弱引用对象有可能已经在 散列表的 weakTable 上，就进行移除
    // 如果曾经设置过一个 weak 对象，后面又设置了一次，就会执行一下代码
    // 抹掉旧的 weak 对象
    if (haveOld) {
    ...
    // 若没有旧值，就意味着这是一个 新值 haveNew，先观察 weak_table ，然后将新的弱引用对象 注册到 weak_table 里面，对新值就行处理
...
objc_retain(id obj)
{
    // 空对象，直接返回
    if (!obj) return obj;
    // 如果是被  isTaggedPointer 标识过，直接返回，证明只是 数据、字符串 基本类型，不是对象
    if (obj->isTaggedPointer()) return obj;
    // 是对象，并retain
    return obj->retain();
}
...
+ (BOOL)isKindOfClass:(Class)cls {
    // 1.Class tcls = object_getClass([NSObject class])的 根元类 与 [NSObject class] 对比， 不匹配
    // 2. tcls = tcls->superclass 变成了 [NSObject class] 与cls（[NSObject class]） 匹配上了
    // 联想一下 isa 的走向图
