...
static void addNamedClass(Class cls, const char *name, Class replacing = nil)
{
    ...
    if ((old = getClass(name))  &&  old != replacing) {
        ...
        // 是旧的 就到这里来 ： 加入到 ：gdb_objc_realized_classes
...
# endif
    ...
    for (EACH_HEADER) {
        ...
        for (i = 0; i < count; i++) {
            ...
            // readClass ：新旧类进行处理与合并，重映射
            Class newCls = readClass(cls, headerIsBundle, headerIsPreoptimized);
            
            // 对懒加载类进行准备处理: 新类 ！=旧类 ，且新类已经存在了(动态运行时获取到了 newCls)
            if (newCls != cls  &&  newCls) {
                // Class was moved but not deleted. Currently this occurs 
                // only when the new class resolved a future class.
                // Non-lazily realize the class below.
                //不是懒加载都会来到这里, 加载类真正加载在下面(realloc)
    ...
    // _getObjc2SelectorRefs 加载文件里的 方法编号 @Sel
    ...
    // _getObjc2ProtocolList 加载文件里的 “协议类方法”
    for (EACH_HEADER) {
...
void prepare_load_methods(const headerType *mhdr)
{
    ...
    // 拿到所有 “类别”,并加载所有 load方法
    // load ,分类没有继承关系，所以不需要递归
...
getMethodNoSuper_nolock(Class cls, SEL sel)
{
    ...
    // fixme nil cls? 
    // fixme nil sel?
    // 从 beginLists 开始 -> endLists 结束
    for (auto mlists = cls->data()->methods.beginLists(), 
              end = cls->data()->methods.endLists(); 
         mlists != end;
         ++mlists)
    {
        // 最终找到了当前的 method 然后 return
...
{
    // 1.因为在上层已经处理了isa指针，并传入了对象 所以 initialize 证明已经被执行过了， initialize 传YES
    // 2.在上层调用后，因为在汇编时没能在cache里找到对应的方法，所以 cache 传 NO
    // 3.
...
/***************************************************** 查找第一步 cache *******************************************************************/
        // imp 获取，为什么要这样设计
        // 通过 cls 在cache查找，ramap(cls) --> 重映射 -->  sel & imp 关联起来
        // 如果能在缓存找到对应的 imp，这样就可以防止后续漫长的查找
    imp = cache_getImp(cls, sel);
    // 找到就直接返回
    if (imp) goto done;

/******************************* 查找第二步 使用isa指向的类，查找methodlist*******************************************************************/
    // Try this class's method lists.
    {
        // 获取方法 但是不从父类解读,在自己所在的方法列表查找
        // 去方法列表查找对应的 imp
        Method meth = getMethodNoSuper_nolock(cls, sel);
        if (meth) {
            // 找到了 method 就给 cache 插入
            log_and_fill_cache(cls, meth->imp, sel, inst, cls);
            // 并给 imp 与 查找到的 imp 赋值
            imp = meth->imp;
            goto done;
        }
    }
    ...
    // 尝试在父类里查找method ,遍历一层一层往上找
    {
        ...
        for (Class curClass = cls->superclass;
             // NSObject 的父类是 nil ，所以不是 NSObject 的父类都会一直查找
             curClass != nil;
    ...
    // 动态解析失败，进入 forwardingTargetForSelector，只能通过汇编查找
...
void *objc_destructInstance(id obj) 
{
    if (obj) {
        ...
        // 判断是否有关联对象： hasAssociatedObjects
        bool assoc = obj->hasAssociatedObjects();

        // This order is important.
        if (cxx) object_cxxDestruct(obj);
        
        // 如果有关联对象， 就进行一系列 remove 操作
